...
# vérifie si le json est bien formaté
def parse(filename):
    try:
...
if os.path.exists(fileconfig):
    ...
    if not data:
        ...
        if 'toleranceSyncro' in data and (data['toleranceSyncro']):
            toleranceSyncro = data['toleranceSyncro']

        if 'facteurPerteSyncro' in data and (data['facteurPerteSyncro']):
            facteurPerteSyncro = data['facteurPerteSyncro']
    ...
    for notified_socket in read_sockets:
        ...
        # Else existing socket is sending a message
            ...
            if z == zMAX:
                ...
                # print log
                logging.info("Vitesse Finale %s km/h", vitesseFinale)

                distanceParcourue += VitBatteur * (vitesseFinale / 3.6)
                print("Distance parcourue = ", distanceParcourue, " / ", distanceAParcourir)

                # print log
                logging.info("Distance Parcourue : %s/%s", distanceParcourue, distanceAParcourir)
